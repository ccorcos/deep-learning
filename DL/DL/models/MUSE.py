...
class MUSE(object):
    """ Multiplicative Unserpervised State Estimator Class
    ...
    f_t = diag( W_fx * x_t ) * ( W_fh * h_tm1 )
    h_t = tanh( W_hf * f_t + W_hx * x_t)

    where x is the "selector" and h is the input


    """

    def __init__(self, rng, obs, act, n_obs, n_act, n_hidden, dropout_rate=0, activation='tanh', outputActivation='softmax', params=None):
