...
class RNNSE(object):
    ...
    A RNNSE is an RNN that has actions and observations. This is an unsupervised model that
    ...
    def __init__(self, rng, obs, act, n_obs, n_act, n_hidden, dropout_rate=0, warmUp=0, ff_obs=[], ff_filt=[], ff_trans=[], ff_act=[], ff_pred=[], activation='tanh', params=None):
        ...
        if params:
            ...
            k0 = theano.shared(value=h0_values, name='k0', borrow=True)

        self.k0 = k0

        # Create the computation graph

        self.k_tm1 = T.matrix() # n_examples, n_hidden @ t-1

        self.o = T.matrix() # n_examples, n_obs @ some specific time
        self.a = T.matrix() # n_examples, n_act @ some specific time
        ...
        self.h = self.hLayer.output

        def obsStep(o_t, k_tm1):
            replaces = {self.o: o_t, self.k_tm1: k_tm1}
            h_t = theano.clone(self.h, replace=replaces)
            return h_t
        ...
        self.k = self.kLayer.output
        self.y = self.outputLayer.output
        
        def actStep(a_t, h_t):
            replaces = {self.a: a_t, self.h: h_t}
            k_t = theano.clone(self.k, replace=replaces)
            y_t = theano.clone(self.y, replace=replaces)
            return k_t, y_t

        # the first observation
        self.k0_t = T.extra_ops.repeat(self.k0[numpy.newaxis, :], obs.shape[0], axis=0)

        h0 = obsStep(self.o[:,0,:], self.k0_t)
        ...
        k_t = T.join([self.k0_t, k_t], axis=0)
        h_t = T.join([self.h0, k_t], axis=0)
        ...
        self.y_pred = T.argmax(self.y_t, axis=2)

        self.outputDistribution = self.y_t
    ...
    def negative_log_likelihood(self, y):
        return -T.mean(T.log(self.y_t)[T.arange(y.shape[0]), T.arange(y.shape[1]), y])
